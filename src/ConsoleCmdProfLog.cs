    ...
    public class ConsoleCmdProfLog : ConsoleCmdAbstract
    {
        ...
        public override string getHelp()
        {
            return "proflog                 show status\n"
                ...
                + "proflog native histcap <n>  sample the histogram if the type has more than n objects, default 50000\n"
            ...
            try
            {
                ...
                if (a == "native")
                {
                    if (_params.Count < 2)
                    {
                        ...
                        Out(NativeProbe.HistLine(NativeProbe.NameTopN));
                    ...
                    if (b == "hist")
                    {
                        ...
                        NativeProbe.HistType = _params[2].ToLowerInvariant();
                        NativeProbe.ApplyHistSelection();
                        Out("native histogram type " + NativeProbe.HistType + ", takes effect at the next census");
                        return;
                    }
                    if (b == "histcap" && _params.Count >= 3)
                    {
                        int hc;
                        if (int.TryParse(_params[2], out hc) && hc >= 0)
                        {
                            NativeProbe.HistCap = hc;
                            Out("native histogram cap " + hc);
                        }
                        else
                        {
                            Out("usage: proflog native histcap <n>");
                        }
                        return;
                    }
                    if (b == "topn" && _params.Count >= 3)
                    {
                        int tn;
                        if (int.TryParse(_params[2], out tn) && tn >= 1 && tn <= 200)
                        {
                            NativeProbe.NameTopN = tn;
                            Out("native histogram topN " + tn);
                        }
                        else
                        {
                            Out("usage: proflog native topn <n>, 1 to 200");
                        }
                        return;
                    }
                    Out("usage: proflog native [now|on|off|every <sec>|cap <n>|names on|off|hist <type>|histcap <n>|topn <n>]");
                    return;
                }
