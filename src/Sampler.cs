...
namespace ProfLog
{
    public static class Sampler
    {
        ...
        private static void OpenWriters()
        {
            ...
            probesWriter.WriteLine("# CullExpired decomposition: CullExpired_ms = CullLockWait + UpdateProtection + GroupTimestamps + scan + RemoveChunks");
            ...
            probesWriter.WriteLine("# CullGroupDirty CullProtDirty count how many CullExpired calls found the respective dirty flag set");
            ...
            nativeWriter.WriteLine("# bundles = loaded AssetBundles; assets pulled from a bundle stay resident until the bundle is unloaded");
            nativeWriter.WriteLine("ts\tuptime_s\tcensus\tcensus_ms\tbundles\ttype\tcount\tbytes\td_count\td_bytes\tsized\tscan_ms");
            nativeWriter.Flush();
        ...
        private static void SayCull(double intervalSec, WorldState st)
        {
            ...
            double gts = Ms((int)Pr.GroupTimestamps);
            double rem = Ms((int)Pr.RemoveChunks);
            double scan = total - wait - prot - gts - rem;
            double keys = extraBuf[(int)Ex.CullKeys] / (double)n;
            double nsPerKey = (keys > 0.0) ? (scan / n) * 1e6 / keys : 0.0;

            Say(string.Format(Inv,
                "cull: {0:F2}ms/call x{1:F2}/s = {2:F0}ms/s | wait={3:F2} prot={4:F2} gts={5:F2} rem={6:F2} scan={7:F2} ({8:F0}ns/key over {9:F0} keys)",
                total / n, n / intervalSec, total / intervalSec,
                wait / n, prot / n, gts / n, rem / n, scan / n, nsPerKey, keys));

            Say(string.Format(Inv,
                "cull ctx: protDirty={0}/{1} gtsDirty={2}/{1} resetReq={3:F0} protLevels={4:F0} groups={5} groupedChunks={6} lockHeld={7:F1}% victims: isInSave={8:F1}/s getSync={9:F1}/s",
                extraBuf[(int)Ex.CullProtDirty], n, extraBuf[(int)Ex.CullGroupDirty],
