...
namespace ProfLog
{
    public static class CullProbe
    {
        ...
        public static int GroupedChunks()
        {
            ...
            {
                RegionFileManager r = Resolve();
                return (r != null && r.groupsByChunkKey != null) ? r.groupsByChunkKey.Count : -1;
            }
            catch
            {
                return -1;
        ...
        public static string Bench(int iters)
        {
            ...
            List<HashSetLong> groups = r.chunkGroups;
            Dictionary<long, HashSetLong> gbk = r.groupsByChunkKey;
            Dictionary<HashSetLong, uint> gts = r.groupTimestamps;
            ...
            int v2N;
            bool anyGroups = gbk != null && gts != null && gbk.Count > 0;
                ...
                foreach (KeyValuePair<long, uint> kv in csd)
                {
                    ...
                    if (resetSet.Count == 0 || !resetSet.Contains(key))
                    {
                        ...
                        if (anyGroups)
                        {
                            HashSetLong g;
                            uint gv;
                            if (gbk.TryGetValue(key, out g) && gts.TryGetValue(g, out gv))
                            {
                                ts = gv;
                            }
                        }
            ...
            double gov = Locked(delegate
            {
                ...
                foreach (long key in csd.Keys)
                {
                    HashSetLong g;
                    uint v;
                    if (gbk.TryGetValue(key, out g) && gts.TryGetValue(g, out v))
                    {
                        c++;
                    }
                }
            ...
            o.Append(" groupedChunks=").Append(gbk != null ? gbk.Count : -1);
            ...
            o.Append(string.Format(Inv, "  group ts override      {0,8:F2} ms   ({1:F0} ns/key)  hits={2}\n", gov, gov * 1e6 / Math.Max(1, keys), govN));
        ...
        public static string Snapshot()
        {
            ...
            try
            {
                ...
                o.Append(" groupedChunks=").Append(r.groupsByChunkKey.Count);
