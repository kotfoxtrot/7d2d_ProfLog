...
> Server-side Harmony mod · 7 Days to Die dedicated server 2.6 · no client download · read-only

## What it does

* times every call site in its patch table — 48 methods in this build — plus an outermost wrapper
...
* censuses live `UnityEngine.Object` counts and their native memory per type, names the individual
  assets behind one chosen type, and lists the loaded `AssetBundle`s — which is where a dedicated
  server's native memory goes and never comes back;
...
Two probes from the 3.x build are absent here because the methods do not exist in 2.6:
`GroupTimestamps` (`UpdateGroupTimestamps`) and `RemoveGrouped` (`RemoveGroupedChunks`). 2.6 keeps
group timestamps current in `SetChunkTimestamp` instead of recomputing them inside the sweep, so the
`CullExpiredChunks` decomposition has one term fewer.
...
1. Download the **2.6** archive from [Releases](https://github.com/kotfoxtrot/7d2d_ProfLog/releases).
...
```bash
git clone -b 2.6 https://github.com/kotfoxtrot/7d2d_ProfLog.git
cd 7d2d_ProfLog
...
proflog native cap <n>         sample instead of sizing every object above n objects (default 50000)
proflog native names on | off  collect the top objects by native size (default on)
proflog native hist <type>|off which type to build the name histogram for (default material)
proflog native histcap <n>     sample the histogram above n objects of that type (default 50000)
proflog native topn <n>        how many names to keep in the histogram (default 25)
...
| `proflog_names_<stamp>.tsv` | the name histogram for the type chosen with `native hist`, one row per name per census |
...
file adds one row per array pool per sample; the native and names files add rows per census, and
censuses default to one every 300 s.
...
The measured cost of `CullExpiredChunks` is far above what a standalone benchmark of the same logic
over the same key count gives. These probes account for the difference, and the answer decides the
fix: if it is the scan, a different loop or an index is needed; if it is lock waiting, throttling is
enough.

| probe | what it settles |
|---|---|
| `AddGrouped` | `AddGroupedChunks` — called when multiblocks are registered, i.e. on chunk load |
...
`grouped_chunks`, `reset_req`, `prot_levels` are the sizes of the chunk group table, the grouped key
set, `resetRequestedChunks` and `chunkProtectionLevels`.

`CullKeys`, `CullResetReq`, `CullProtLevels` are sums over all `CullExpired` calls in the interval;
for the average, divide by `CullLockWait_n`. `CullProtDirty` counts how many calls found
`protectionLevelsDirty` raised.
...
| column | meaning |
|---|---|
| `sized` | `1` every object of that type was sized, `2` the type exceeded `native cap` so the bytes were sampled with a fixed stride and extrapolated, `0` not sized at all |
| `instances` | objects whose name carries Unity's `(Instance)` suffix, i.e. runtime copies rather than loaded assets; `-1` means not measured for that type |

`proflog native hist <type>` picks one type (default `material`) and writes a name histogram for it
into `proflog_names_<stamp>.tsv`: every trailing `(Instance)` suffix is stripped, so runtime copies
group under the asset they came from, and `d_count` per name is what points at the specific leaking
asset. Above `native histcap` objects the histogram is sampled with a fixed stride and scaled up,
which the `est=1` column marks.
...
— on an Undead Legacy server the bundles alone are about 6.7 GB. `proflog bundles` lists them.
...
```
cull: 17.53ms/call x9.42/s = 165ms/s | wait=.. prot=.. rem=.. scan=.. (..ns/key over .. keys)
cull ctx: protDirty=n/N resetReq=.. protLevels=.. groups=.. groupedChunks=.. lockHeld=..% victims: isInSave=../s getSync=../s
```

`scan` is the remainder: `CullExpired − CullLockWait − UpdateProtection − RemoveChunks`. If `scan` is
large, a different loop is needed. If `wait` is large, the problem is lock contention.

## `proflog cullbench [n]`

Runs four loop variants **on live data, inside the game's Mono, on the production CPU**:
...
`cullbench|` prefix. `V0` is what the game actually runs; the measured `CullExpired` mean sits in the
probe TSV.

Run it under load, while there are people in the game — otherwise the numbers will be those of an
empty world.

`proflog cull` is the instant snapshot next to it: the sizes of all the collections involved, the
dirty flag, `maxChunkAge`, `maxBytes`, and the state of `lockprobe`.
...
- A `sized=2` row is an extrapolation from a sample, not a measurement. Compare those against
  `native_res_mb` before drawing conclusions from the absolute bytes.
...
| mod | what it is for |
|---|---|
| [MaxChunkAgeDeadlockFix](https://github.com/kotfoxtrot/7d2d_MaxChunkAgeDeadlockFix) | makes `MaxChunkAge` chunk reset safe by breaking the lock-order deadlock that freezes the server; the 2.6 build also repairs the region file and shared chunk stream synchronization 2.6 omits |
...
own server: `CullExpired`, `CullLockWait`, `UpdateProtection` and `RemoveChunks` measure exactly what
CullExpiredFix changes, and `MultiBlockMain` covers MaxChunkAgeDeadlockFix's main-thread drain. It is
useful next to them and not required by either.
